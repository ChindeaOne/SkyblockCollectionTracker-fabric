...
object DwarvenHeatmap {
        ...
        for (x in -1..1) {
            for (y in -1..1) {
                for (z in -1..1) {
                    ...
                    val block = state.block
                    val keyName = BuiltInRegistries.BLOCK.getKey(block) ?: continue
                    if (blockList.contains(keyName.toString())) score++
                }
            }
        }
        return score
    ...
    private fun priorityColor(score: Int): Triple<Float, Float, Float> {
        return when {
            score >= 10 -> Triple(4f / 255f, 4f / 255f, 4f / 255f) // black
            score >= 8 -> Triple(49f / 255f, 41f / 255f, 165f / 255f) // dark blue
            score >= 6 -> Triple(104f / 255f, 210f / 255f, 249f /255f) // blue
    ...
    private fun isBlockExposed(world: ClientLevel, pos: BlockPos): Boolean {
        ...
        fun isNotSolid(pos: BlockPos): Boolean {
            val state = world.getBlockState(pos)
            return state.isAir || state.block == Blocks.SNOW
        }
